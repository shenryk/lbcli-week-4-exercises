# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

utxo_txid="23c19f37d4e92e9a115aab86e4edc1b92a51add4e0ed0034bb166314dde50e16"
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

changeaddress=$(bitcoin-cli -regtest -rpcwallet="btrustwallet" getrawchangeaddress)
locktime=2041

rawtxhex=$(bitcoin-cli -named createrawtransaction   inputs='''[ "txid": "'$utxo_txid'", "vout": 0, "sequence": 4294967294 },{ "txid": "'$utxo_txid'", "vout": 1, "sequence": 4294967294 }]''' outputs='''{"'$recipient'": 0.20000000,"'$changeaddress'": 0.03678108}''' locktime=$locktime)

echo "$rawtxhex"
