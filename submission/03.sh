# Create a raw transaction and add this message in it: "btrust builder 2026"

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

utxo_txid="23c19f37d4e92e9a115aab86e4edc1b92a51add4e0ed0034bb166314dde50e16"
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

changeaddress=$(bitcoin-cli -regtest -rpcwallet="btrustwallet" getrawchangeaddress)
message_hex=$(echo -n "btrust builder 2026" | xxd -ps -c 200)   # 627472757374206275696c6465722032303236

rawtxhex=$(bitcoin-cli -regtest -named createrawtransaction inputs='''[{ "txid": "'$utxo_txid'", "vout": 0 },{ "txid": "'$utxo_txid'", "vout": 1 }]'''outputs='''{"'$recipient'": 0.20000000,"'$changeaddress'": 0.03678108,"data": "'$message_hex'"}''')

echo "$rawtxhex"
